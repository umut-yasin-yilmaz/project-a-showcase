...
| R2 | **Denge kırılması** — ekonomi veya spawn HP ile uyumsuz | Yüksek | Orta | “Çok kolay / imkânsız” iç test notları artıyor | `mvp_balance_goals.md` + data-driven tablolar; tek seferde tek eksen değiştir | Sen |
| R3 | **Determinizm / RNG dağılması** — farklı build’lerde farklı sonuç | Orta | Yüksek | `randf()` ile outcome; kaydedilemeyen run | Run-scoped RNG; kurallar: `godot-brotato-ai-workflow.mdc` | Sen + AI |
...
| R6 | **Steam / export sürprizi** — geç build hatası | Orta | Yüksek | İlk export denemesi çok geç | `plan_mvp.md` D3; export preset erken | Sen |
